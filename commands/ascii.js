...
module.exports = {
    ...
    run: async (message, args) => {
        ...
        if (list.includes(message.member.user.id)) {
            return message.channel.send("<a:1603_Animated_Cross:716318362644381757> You have opted out of bot DM's, use /optin to enable this command");
        }
        ...
        if (args.length == 0) {
            const embed = new MessageEmbed()
                ...
                .addField("Help", "to view different fonts use /ascii fonts <(1/2/3/4/5)>")
                .setColor(color)
                .setFooter("Created By Jeremy#6414")
            return message.channel.send(embed).catch(() => {
                return message.channel.send("<a:1603_Animated_Cross:716318362644381757> You must include some text");
            })
        }
        ...
        if (args[0] == "fonts") {
            if (args.length == 1) {
                return message.channel.send("<a:1603_Animated_Cross:716318362644381757> /ascii fonts <(1/2/3/4/5)>")
            } else if (args[1] == "1") {
                const embed = new MessageEmbed()
        ...
        await ascii(text, {
            ...
            if (!err) {
                ...
                fail = true
                return message.channel.send("<a:1603_Animated_Cross:716318362644381757> Error - maybe an incorrect font - fonts are **cAsE sEnSiTiVe**")
            }
        });
        ...
        setTimeout(() => {
            ...
            if (asciiString.length >= 2000) {
                return message.channel.send("<a:1603_Animated_Cross:716318362644381757> Ascii text exceeds discord message size");
            }
            
            message.member.send(asciiString).then( () => {
                return message.channel.send("<a:6093_Animated_Checkmark:716318362719879288> Success! **-** Check your DM's!");
            }).catch( () => {
                return message.channel.send("<a:1603_Animated_Cross:716318362644381757> Unable to send you a DM!");
            });
        }, 500);
